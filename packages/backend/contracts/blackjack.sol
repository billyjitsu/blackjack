...
contract Blackjack {
    ...
    function bet (uint256 _betAmount) public payable {
        require(msg.value >=_minAmount, "Bet amount must be equal to msg.value");
        dealCards();
    ...
    generateRandomNumber() public returns (uint256) {
        //chainlink or api3
        uint256 randomnumber = uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, msg.sender))) % 13;
        return randomnumber;
    }
